...
`Note:` *You will need a MySQL database to be able to use Insane Announcer in proxy mode. Depending on your host, the process can vary, and will not be mentionned nor explained here.*
...
## Connecting to MySQL
As mentionned above, a MySQL database is needed to sync Insane Announcer accross servers. To start your syncing process, connect to your server in-game, and open the plugin's GUI by executing the command `/announcer` in chat.
<br>
...
`/announcer >> Settings >> MySQL Database >> Setup MySQL >> Credentials`
